...
class StudentFeeSchedule(Document):
	...
	def after_save(self):
		...
		for component in self.components:
			fee_categories.append(frappe.get_doc("Fee Category", component.fees_category))
		...
		for student in self.fee_schedule_student:
			...
			for category in fee_categories:
				month_condition = ""
				if category.is_recurring and self.due_date:
					month_condition = " and month(fees.due_date)={}".format(frappe.db.escape(due_month))

				category_condition = " and sfc.fees_category='{}'".format(frappe.db.escape(category.name))
				...
				fees = frappe.db.sql("""
					select sfc.fees_category, fees.student_name, fees.student
						...
						where
							...
							{month_condition}
							{category_condition}
					""".format(conditions=conditions, student_condition=student_condition, month_condition=month_condition, category_condition=category_condition), (academic_year), as_dict=1)
				...
				if student_group.student_group:
					...
					for student in students:
						...
						if exist == False:
							...
							due_month = frappe.utils.formatdate(self.get("due_date"), 'MM')
							...
							for component in self.get("components"):
								if component.student_category == student.category or not component.student_category:
									fee_category_doc = frappe.get_doc("Fee Category", component.fees_category)
									fee_categories.append({
										'category':component.student_category, 
										'name':component.fees_category, 
										'is_recurring':fee_category_doc.is_recurring, 
										'is_transport':component.is_transport,
										'amount':component.amount
									})

							student_total = 0
							student_transport_total = 0
							...
							for category in fee_categories:
								...
								if not len(fees):
									if category['category'] and not category['is_transport']:
										student_total += flt(category['amount'])
									elif not category['category'] and not category['is_transport']:
										student_total += flt(category['amount'])
									elif category['category'] and category['is_transport']:
										student_transport_total += flt(category['amount'])
									elif not category['category'] and category['is_transport']:
										student_transport_total += flt(category['amount'])
							...
							if student.category and (student.transportation == "School Bus" or student.transportation == "باص المدرسة"):
								student_row.amount = flt(student_total) + flt(student_transport_total)
								...
								student_row.amount = flt(student_total)
	...
	for student in students:
		if student.student not in generated_fees:
			...
			try:
				...
				due_month = frappe.utils.formatdate(doc.due_date, 'MM')
				...
				for category in fee_categories:
					month_condition = ""
					if category['is_recurring'] and doc.due_date:
						month_condition = " and month(fees.due_date)={}".format(frappe.db.escape(due_month))
				...
				#Add student Row
				if (student_total + student_transport_total) > 0:
					for component in doc.components:
						valid = True
						for student_fee in student_fees:
							if component.fees_category == student_fee.fees_category and (component.student_category == student.category or not component.student_category):
								valid = False
